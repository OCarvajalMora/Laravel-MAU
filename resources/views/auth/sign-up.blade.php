...
<head>
    ...
    <title>Create a new account | ManagApp</title>
...
<body>
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
          ...
          <div>
            @if ($errors->any())
              <div class="bg-red-100 p-3 border rounded border-red-200 text-sm">
                <strong>Ops!</strong> We really sorry, but:
                  <ul class="mt-1">
                      @foreach ($errors->all() as $error)
                          <li class="font-small px-2">- {{ $error }}</li>
                      @endforeach
                  </ul>
              </div>
            @endif
          ...
          <form class="mt-8 space-y-6" action="{{ route('register')}}" method="POST">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                  ...
                  <input id="username" name="username" value="{{ old('username') }}" type="text" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Username">
                </div>
                <div>
                  <label for="email-address" class="sr-only">Email address</label>
                  <input id="email-address" name="email" value="{{ old('email') }}" type="email" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Email address">
                </div>
                <div>
